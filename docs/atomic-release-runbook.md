...
`vite build`). A `RELEASE_MANIFEST.txt` records the `source_sha`, build metadata, and a SHA-256 over
each `dist` tree. The prod host has **no node/npm**, so the frontend is built **locally** — there is
**no server-side `npm ci` / `npm run build`**. The bundle excludes `.git`, `node_modules`, and the
...
2. **Manifest + frontend dist integrity** — manifest present with `source_sha == $REF`, both `dist`
   non-empty, and each `dist` SHA-256 recomputed on the server must match the manifest → else **ABORT
   (exit 24)**. (Assets are built locally; this proves the shipped assets weren't corrupted.)
...
WORK=$(mktemp -d); SRC=$WORK/src
git -c core.autocrlf=false -c core.eol=lf archive --format=tar <REF> | tar -x -C $SRC
                                                               # exact committed tree; LF forced so a Windows
                                                               # core.autocrlf=true checkout can't inject CRLF and
                                                               # trip the drift-gate vs the LF prod files.
...
write $SRC/RELEASE_MANIFEST.txt: source_sha=<full>, built_at, node, npm, <sha256 of each dist tree>
...
verify_bundle: source == <REF> tracked tree; both dist present+nonempty; manifest present;
               NO real .env / node_modules / .git   (else ABORT exit 13)
...
# ---- MANIFEST + FRONTEND DIST INTEGRITY (assets built locally; NO server build) ----
ssh flashbot: test manifest source_sha == <full>; both <stage>/{miniapp,admin}/dist/index.html non-empty;
              recompute sha256 of each dist on the server == manifest values   (else ABORT exit 24)
