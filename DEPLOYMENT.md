...
# --- AI чат через OmniRoute (главный путь — см. §6.5) ---
AI_BASE_URL_ALLOWLIST=omniroute   # ОБЯЗАТЕЛЬНО для внутреннего шлюза (иначе SSRF-блок)
...
## 6.5. Чат через OmniRoute (главный путь запуска)

Текстовый чат бота идёт через AI-аккаунт. Шлюз OmniRoute уже в
`docker-compose.yml` — приложение видит его внутри сети как `http://omniroute:20128/v1`.
Чтобы чат заработал:

1. **Открой allowlist для внутреннего шлюза** (ОБЯЗАТЕЛЬНО) — без этого SSRF-защита
   отклонит внутренний адрес с ошибкой «base_url resolves to a non-public address»:
   ```dotenv
   AI_BASE_URL_ALLOWLIST=omniroute
   ```
...
3. **OmniRoute**: открой его дашборд (в проде — `omniroute.{$DOMAIN}` под IP-allowlist),
   подключи провайдера(ов) и создай в нём API-ключ.
4. **Добавь аккаунт в админке**: AI-роутинг → новый аккаунт →
   ...
   (Также можно `kind=openrouter`/`openai`/`custom` с прямым ключом провайдера.)
